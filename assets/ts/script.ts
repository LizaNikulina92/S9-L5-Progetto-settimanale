...
let credito1: number = Number(prompt('Credito Utente-1'));
let credito2: number = Number(prompt('Credito Utente-2'));
let credito3: number = Number(prompt('Credito Utente-3'));
class Smartphone {
    id: string;
    carica: number;
    numChiamate: number;
    ...
    public chiamata(): Array<any> {
        this.sec++;
        if (this.sec >= 60) {
            this.sec = 0;
            this.min++;
        }
        let opCredito: number = (this.carica - ((this.min + 1) * 0.2));
        return [`Minuti totali delle telefonate: 
                ${this.min} : ${(this.sec)}`, opCredito]
    }
    public numero404(): number {
        let crRes: number = (this.carica - ((this.min + 1) * 0.2)) - 1;
        return crRes;
    }
}
...
let arrUsers: Smartphone[] = [firstUser, secondUser, thirdUser];
function createSmartpgone() {
    arrUsers.forEach(user => {
        ...
        let arrTastiera: Array<any> = [1, 2, 3, 4, 5, 6, 7, 8, 9, '*', 0, '#'];
        let arrNumeri: Array<any> = [];
        let arrDigit: Array<any> = [];
        let timer: number;
        let moneys: number;
        ...
        let inputNumber: any = document.createElement('input');
